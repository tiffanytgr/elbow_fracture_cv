...
def _case_id(ap_path: Optional[str], lat_path: Optional[str]) -> Optional[str]:
    """Case identifier built from the uploaded X-ray file path(s)."""
...
async def predict(
    ...
    ap_source_path / lat_source_path : original client-side file path of each
        image, used as the case identifier in the log (falls back to filename)
    """
    ...
    _log_prediction(
        case_id=_case_id(ap_log_path, lat_log_path),
        ap_path=ap_log_path,
